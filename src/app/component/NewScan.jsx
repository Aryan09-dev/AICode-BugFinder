...
import { websiteAnalyze } from "../../services/scanservice";
...
const NewScan = () => {
  ...
  const handleStartScan = async () => {
    if (!url || !isValidWebsite(url)) {
      alert(
        "Please enter a real website (like https://google.com/ or https://example.com/)",
      );
      return;
    }
    ...
    try {
      ...
      const response = await websiteAnalyze(scanData);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="p-8 bg-white rounded-xl shadow-sm">
        <h2 className="text-3xl font-montserrat mb-6">
          Start New Website Analysis
        </h2>
        ...
        {/* <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-4 ">
          {Object.entries(scanOptions).map(([key, value]) => (
            <label
              key={key}
              className={`flex items-start font-robotoCondensed gap-3 p-4 border rounded-lg hover:border-brandBlue cursor-pointer ${
                key === "accessibility" ? "md:col-span-2" : ""
              }`}
            >
              <input
                type="checkbox"
      ...
      <div className="p-6 bg-blue-100 rounded-xl shadow-sm">
        <h3 className="text-2xl mb-4 font-montserrat font-medium">
          What we analyze:
        </h3>
