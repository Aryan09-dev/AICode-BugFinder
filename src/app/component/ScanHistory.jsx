  ...
  useEffect(() => {
    const fetchScans = async () => {
      try {
        ...
        const formatted = response.data.map((scan) => ({
          id: scan.scan_Id,
          websiteUrl: scan.website_Url,
          scanDate: new Date(scan.created_On).toLocaleDateString(),
          scanTime: new Date(scan.created_On).toLocaleTimeString(),
          ...
          status: scan.scan_Status.toLowerCase(),
        }));
    ...
    return (
      <DashboardLayout currentPage="scan-history">
        <div className="flex justify-center items-center h-64">
          <button
            disabled
            className="bg-brandBlue text-white px-6 py-3 rounded-lg flex items-center gap-2"
          >
            Loading Scans...
          </button>
        </div>
    ...
    <DashboardLayout currentPage="scan-history">
      <div className="space-y-6 max-w-6xl mx-auto">
            ...
            <tbody>
              {filteredScans.map((scan) => (
                <tr key={scan.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-3 text-blue-600">{scan.websiteUrl}</td>
        ...
        {filteredScans.length === 0 && (
          <p className="text-center text-gray-500">No scans found.</p>
        )}
      </div>
    </DashboardLayout>
  );
